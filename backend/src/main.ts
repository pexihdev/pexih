        ...
        .where(
          and(
            ...
            or(
              ...
              lte(articles.publishedAt, Date.now()),
            ),
          ),
        )
        .orderBy(desc(articles.createdAt));
...
app.get("/api/articles/realtime-stats", async (req, res) => {
  try {
    ...
    const approvedCount = totalArticles.filter(
      (a) => a.status === "approved",
    ).length;
    const pendingCount = totalArticles.filter(
      (a) => a.status === "pending",
    ).length;
    const viewsSum = totalArticles.reduce(
      (sum, a) => sum + (parseInt(a.views) || 0),
      0,
    );
...
app.get("/api/articles/:id", async (req, res) => {
  try {
    ...
    if (resDb.length === 0) {
      return res
        .status(404)
        .json({ error: `Article with ID ${id} not found.` });
    }
...
app.get("/api/analytics", async (req, res) => {
  try {
    ...
    const numDays = parseInt(daysStr as string) || 7;

    // Get all articles for total views and top articles
    const allArticles = await db
      .select({
        id: articles.id,
        title: articles.title,
    ...
    const top3 = sortedArticles.slice(0, 3);

    // Mock trend data for top 3 articles based on their total views over N days
    const daysList = [];
    const now = new Date();
    ...
    const trendData = daysList.map((day, index) => {
      // Simulate an organic trend curve using sine waves and randomness
      const progress = index / (numDays - 1); // 0 to 1
      const obj: any = { day };
      top3.forEach((art, i) => {
        const key = `article${["A", "B", "C"][i]}`;
        const total = parseInt(art.views) || 0;
        // Distribute total views across days somewhat organically
        const baseShare = total / numDays;
        const variation = Math.sin(progress * Math.PI + i) * 0.3 + 1; // 0.7 to 1.3
        let val = Math.floor(
          baseShare * variation * (0.8 + Math.random() * 0.4),
        );
        obj[key] = Math.max(10, val); // minimum 10 views
      });
      return obj;
    });

    const titles = {
      articleA: top3[0]?.title || "Artikel 1",
      articleB: top3[1]?.title || "Artikel 2",
      articleC: top3[2]?.title || "Artikel 3",
    };
    ...
    allArticles.forEach((a) => {
      ...
      catMap[c] = (catMap[c] || 0) + (parseInt(a.views) || 0);
    });
    ...
    const totalCatViews = categoryData.reduce((s, c) => s + c.value, 0);
    categoryData.forEach((c) => {
      c.value =
        totalCatViews > 0 ? Math.round((c.value / totalCatViews) * 100) : 0;
    });

    // Retention data (Mocked based on real users count)
    ...
    for (let i = 1; i <= 4; i++) {
      retentionData.push({
        week: `Minggu ${i}`,
        newUsers: Math.floor(totalUsers * (Math.random() * 0.3 + 0.1)),
        returningUsers: Math.floor(
          totalUsers * (Math.random() * 0.6 + 0.4) * i,
        ), // growing returning users
      });
    }
    ...
    res.json({
      stats: {
        ...
        readers: (totalUsers * 142).toLocaleString(), // proxy for unique readers
        duration: "3m 12s",
        bounce: "38.4%",
      },
      trendData,
  ...
  try {
    ...
    if (data.id) {
      ...
      existing = await db
        .select()
        .from(categories)
        .where(eq(categories.title, data.title))
        .limit(1);
    }
...
app.post("/api/auth/login", rateLimit(5, 60), async (req, res) => {
  try {
    ...
    // Auto-seed admin if no users exist and trying to login as admin
    if (isAdminLogin && email === "admin@pexih.com") {
      ...
      const existing = await db
        ...
        .where(
          and(
            ...
            eq(articleLikes.username, uname),
          ),
        )
        .limit(1);
      if (existing.length === 0) {
        await db
          .insert(articleLikes)
          .values({ username: uname, articleId, createdAt: Date.now() });
      }
  ...
  try {
    ...
    const userLikes = await db
      ...
      .where(
        and(
          eq(articleLikes.articleId, articleId),
          eq(articleLikes.username, username),
        ),
      )
      .limit(1);
    const userBookmarks = await db
      ...
      .where(
        and(
          eq(bookmarks.articleId, articleId),
          eq(bookmarks.username, username),
        ),
      )
      .limit(1);
  ...
  try {
    ...
    if (revisionId) {
      const rev = await db
        .select()
        .from(articleRevisions)
        .where(eq(articleRevisions.id, Number(revisionId)))
        .limit(1);
      if (rev.length > 0) {
  ...
  try {
    ...
    if (!apiKey || !apiSecret) {
      ...
      return res.json({
        success: true,
        url: dataUrl,
        message: "Cloudinary credentials missing, using Base64 fallback",
      });
    }
    ...
    if (!cloudinaryResponse.ok) {
      return res
        ...
        .json({ error: result.error?.message || "Cloudinary upload failed" });
    }
...
app.delete("/api/media/:id", async (req, res) => {
  try {
    ...
    if (isNaN(mediaId)) return res.status(400).json({ error: "Invalid ID" });

    // Optional: we could delete from Cloudinary here too using the admin API
    // but for now just removing it from DB to save space in the list
...
if (!process.env.VERCEL) {
  ...
  app.listen(port, () => {
    console.log(
      `🚀 Yondaime Node.js Express server running on: http://localhost:${port}/api`,
    );
  });
}
