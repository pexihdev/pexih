...
### Isolasi Tata Letak Mobile & Desktop (Anti-Tabrakan)
- **Pemisahan Kelas CSS & Tinggi (Strict Layout Isolation)**:
  - **Mobile Layout**: Menggunakan pembungkus luar bertinggi tetap (`fixed inset-0 max-w-[400px] flex flex-col`) dengan pengaturan `style="overflow: hidden !important;"`. Konten di dalamnya WAJIB digulung melalui container khusus bertinggi dinamis/penuh seperti `#mobile-scroll-container` atau `#admin-mobile-scroll-container` dengan class `flex-1 h-full overflow-y-auto`. Jangan sekali-kali mengubah container mobile menjadi `h-screen` atau `min-h-0` tanpa pengujian scroll menyeluruh.
  - **Desktop Layout**: Menggunakan struktur kontainer penuh tanpa batasan lebar (`w-full flex-1 flex flex-col min-h-0 overflow-y-auto`). Pembungkus utama menggunakan id `#desktop-main-wrapper` dengan class `flex-1 min-h-0 w-full flex flex-col overflow-y-auto`.
- **Independensi DOM & Identitas Unik (No Property Leakage)**:
  - Setiap perubahan CSS, penambahan script interaktif, atau modifikasi event listener (misalnya pada penanganan scroll, efek parallax, progress membaca, pull-to-refresh) di versi Mobile HARUS diisolasi menggunakan ID unik (seperti `#mobile-scroll-container`) dan TIDAK BOLEH memengaruhi atau menggunakan elemen global yang dapat memicu kegagalan di versi Desktop.
  - Setiap komponen atau halaman baru yang dibuat secara paralel untuk Mobile dan Desktop harus merujuk ke Layout masing-masing (`MobileLayout` / `AdminMobileLayout` vs `DesktopLayout` / `AdminDesktopLayout`). Jangan mencampurkan layout tersebut.
  - Perubahan gaya visual (gaya kartu, bayangan, border-radius) di halaman Mobile harus diatur secara terpisah tanpa mengubah tampilan di halaman Desktop `/desktop`, kecuali jika diminta secara eksplisit. Aturan ini juga berlaku untuk panel Admin Mobile (`/admin`) dan Admin Desktop (`/admin/desktop`).
...
- **Top Header Navbar**: Every header for mobile, desktop frontend, and admin panels must use exactly padding `pt-3 pb-2 px-5`. This applies to both the Home page and inner pages.
- **Bottom Navigation Bar (Mobile frontend & admin panel)**: Height must be exactly `h-[52px]`, with label text sized `text-[8px]`, and icons sized `w-4 h-4`. Highlight active tabs using Orange color styling.
